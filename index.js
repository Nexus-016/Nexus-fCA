...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  // Schedule mid-session lightweight token poke (~ every 6-7h) to keep cookies warm
        if(!globalOptions._lightRefreshTimer){
          const scheduleLight = () => {
            const base = 6 * 60 * 60 * 1000; // 6h
            // jitter 0..60min -> results in 6..7h window
            const jitter = Math.random() * 60 * 60 * 1000; // 0-60m
